...
void gArta_StackNodes_free(gArta_StackNode node, gArta_Data (*destroy_data)(gArta_Data));

void gArta_StackNode_print(int *print_pt, const gArta_StackNode node, int (*print_data)(const gArta_Data));
void gArta_StackNodes_print(int *print_pt, const gArta_StackNode node, int (*print_data)(const gArta_Data));
...
gArta_Stack gArta_Stack_destroy(gArta_Stack stack)
{
    ...
    gArta_StackNodes_free(stack -> top, stack -> dataInfos_pt -> destroy);
    if (!gArta_Error_global_isNone()) { return stack; }
    stack -> top = NULL;
...
int gArta_Stack_print(const gArta_Stack stack)
{
    ...
    gArta_StackNodes_print(&print_tmp, stack -> top, stack -> dataInfos_pt -> print);
...
void gArta_StackNodes_free(gArta_StackNode node, gArta_Data (*destroy_data)(gArta_Data))
{
    ...
    while (node != NULL) {
        ...
        gArta_StackNode_free(node_tmp, destroy_data);
        if (!gArta_Error_global_isNone()) { return; }
...
void gArta_StackNodes_print(int *print_pt, const gArta_StackNode node, int (*print_data)(const gArta_Data))
{
    ...
    gArta_StackNode_print(&print_tmp, node_tmp, print_data);
    if (!gArta_Error_global_isNone()) { *print_pt = print_tmp; return; }
    print_val += print_tmp;
